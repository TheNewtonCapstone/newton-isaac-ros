...
# Find the section in the script where DOCKER_ARGS are set for aarch64 platform
if [[ $PLATFORM == "aarch64" ]]; then
    # Replace the problematic line
    # DOCKER_ARGS+=("-e NVIDIA_VISIBLE_DEVICES=nvidia.com/gpu=all,nvidia.com/pva=all")
    # With these traditional device mounts
    DOCKER_ARGS+=("-v /dev/nvhost-ctrl:/dev/nvhost-ctrl")
    DOCKER_ARGS+=("-v /dev/nvhost-ctrl-gpu:/dev/nvhost-ctrl-gpu")
    DOCKER_ARGS+=("-v /dev/nvhost-prof-gpu:/dev/nvhost-prof-gpu")
    DOCKER_ARGS+=("-v /dev/nvmap:/dev/nvmap")
    DOCKER_ARGS+=("-v /dev/nvhost-gpu:/dev/nvhost-gpu")
    DOCKER_ARGS+=("-v /dev/nvhost-as-gpu:/dev/nvhost-as-gpu")
    DOCKER_ARGS+=("-v /dev/nvhost-vic:/dev/nvhost-vic")
    DOCKER_ARGS+=("-v /dev/nvhost-pva0:/dev/nvhost-pva0")
    DOCKER_ARGS+=("-v /dev/nvhost-pva0-engine:/dev/nvhost-pva0-engine")
    
    # Keep the rest of the existing mounts
